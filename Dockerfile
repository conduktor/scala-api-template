...
RUN if [ -n "${upx_compression}" ]; then \
      ...
      upx-3.96-amd64_linux/upx "$upx_compression" /build/target/graalvm-native-image/api-template; \
    fi

RUN echo 'unprivileged:x:65534:65534:unprivileged:/:' > /etc/passwd.minimal

FROM scratch
COPY --from=builder /build/target/graalvm-native-image/api-template /server
